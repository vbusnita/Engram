...
| 5 | `append_notes` | write | ✅ | `POST /mcp/append_notes` | Timestamped append to the neuron body. The cross-session memory primitive — scouts use it to leave evidence and reasoning for the next scout. Each call adds a new H3 block with ISO timestamp + agent byline. Hard rejects payloads >64 KB; tool description guides agents to keep per-call notes under 4 KB. NOT idempotent. Returns 404 if neuron doesn't exist (no auto-create). Shipped 2026-05-20. |
